...
<template>
  <div mb5 flex justify-between items-center>
    <input
      v-model="organizationStore.searchQuery"
      animation
      text-field
      type="text"
      placeholder="Search ..."
  ...
  <div v-auto-animate grid lg:grid-cols-2 md:grid-cols-1 md:pr10 lg:pr0 gap3 w-full>
      ...
      >
        ...
        <div w-full h-full flex justify-between>
          ...
          <div flex flex-col gap2 items-end>
            <div opacity-50>
              {{ `Created: ${organization.created_at.getDate()}/${organization.created_at.getMonth()}/${organization.created_at.getFullYear()}` }}
            </div>
    ...
    >
      <template v-if="selectedOrganization">
        <OrganizationModal v-model:organization="selectedOrganization" v-model:rsakey="selectedOrganization.hashed_key" @close="toggleModal(selectedOrganization)" />
      </template>
    </transition>
  </div>
</template>
