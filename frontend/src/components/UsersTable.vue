...
import { Roles, UserRetrieve } from '~/models/user'

const userStore = useUsersStore()
const organizationStore = useOrganizationStore()

const selectedUser: Ref<UserRetrieve | undefined> = ref()
...
const toggleModal = (organization: UserRetrieve | undefined) => {
...
<template>
  <div mb5 mt5 flex justify-between items-center>
    <div flex gap3 items-center>
      ...
      <ComboBox
        v-if="organizations" v-model:value="filteredOrganization" :iterable="organizations as [string]"
  ...
  <div v-auto-animate grid lg:grid-cols-2 md:grid-cols-1 md:pr10 lg:pr0 gap3 w-full>
    <template v-for="user in userStore.users" :key="user.id">
      <div
        ...
        @click="toggleModal(user as UserRetrieve)"
      >
